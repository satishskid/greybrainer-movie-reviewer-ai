...
import { getGroqApiKey, isValidGroqKeyFormat, removeGroqApiKey, storeGroqApiKey, updateGroqKeyValidation } from '../utils/groqKeyStorage';

interface GroqKeyManagerProps {
  className?: string;
}

export const GroqKeyManager: React.FC<GroqKeyManagerProps> = ({ className = '' }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      const isValidFormat = isValidGroqKeyFormat(apiKey.trim());
      await storeGroqApiKey(apiKey.trim(), isValidFormat);
      await updateGroqKeyValidation(isValidFormat);

      setMessage({
        type: 'success',
        text: isValidFormat ? 'Groq API key saved successfully!' : 'Groq key saved (format validation failed)',
      });
  ...
  const handleRemove = () => {
    if (window.confirm('Are you sure you want to remove your stored Groq API key?')) {
      removeGroqApiKey();
      setMessage({ type: 'success', text: 'Groq API key removed successfully' });
      checkStoredKey();
      setApiKey('');
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-slate-800 rounded-xl shadow-2xl p-6 w-full max-w-md border border-slate-700">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">Current Key Status</label>
              ...
              <div>
                <label htmlFor="groqApiKey" className="block text-sm font-medium text-slate-300 mb-2">API Key</label>
